...
const PaymentsCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            sessionId: _entity?.sessionId,paymentStatus: _entity?.paymentStatus,paymentType: _entity?.paymentType,amount: _entity?.amount,currency: _entity?.currency,paymentIntendId: _entity?.paymentIntendId,payedAt: _entity?.payedAt,
