...
import tenantServicesFakerFactory from "./tenantServicesFakerFactory";
...
const TenantServicesFakerDialogComponent = (props) => {
...
client.service("payments")
                        .find({ query: { $select: ["_id","sessionId"] } })
                        .then((data) => {
...
client.service("services")
    ...
    const onRun = async () => {
        let fakeData = tenantServicesFakerFactory(props.user,fakerCount,tenantIdIds,paymentIdIds,serviceIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("tenantServices").create(elem));
...
export default connect(mapState, mapDispatch)(TenantServicesFakerDialogComponent);
