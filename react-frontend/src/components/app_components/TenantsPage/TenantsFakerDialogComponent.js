...
import tenantsFakerFactory from "./tenantsFakerFactory";
...
const TenantsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = tenantsFakerFactory(props.user,fakerCount,partnerIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("tenants").create(elem));
...
export default connect(mapState, mapDispatch)(TenantsFakerDialogComponent);
