...
import tenantInvoicesFakerFactory from "./tenantInvoicesFakerFactory";
...
const TenantInvoicesFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [tenantIdIds, settenantIdIds] = useState();
const [usageLogsIds, setusageLogsIds] = useState();


    useEffect(() => {
        setFakerCount(1);
       
        client.service("tenants")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        settenantIdIds(data.data);
                        });
client.service("usageLogs")
                        .find({ query: { $select: ["_id","requestId"] } })
                        .then((data) => {
                        setusageLogsIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = tenantInvoicesFakerFactory(props.user,fakerCount,tenantIdIds,usageLogsIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("tenantInvoices").create(elem));
...
export default connect(mapState, mapDispatch)(TenantInvoicesFakerDialogComponent);
