...
import TenantInvoicesDatatable from "./TenantInvoicesDataTable";
import TenantInvoicesEditDialogComponent from "./TenantInvoicesEditDialogComponent";
import TenantInvoicesCreateDialogComponent from "./TenantInvoicesCreateDialogComponent";
import TenantInvoicesFakerDialogComponent from "./TenantInvoicesFakerDialogComponent";
import TenantInvoicesSeederDialogComponent from "./TenantInvoicesSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const TenantInvoicesPage = (props) => {
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("tenantInvoices");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , tenantId : urlParams.singleTenantsId ,usageLogs : urlParams.singleUsageLogsId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Tenant Invoices", type: "error", message: error.message || "Failed get Tenant Invoices" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("tenantInvoices").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Tenant Invoices", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("tenantInvoices").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/tenantInvoices/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="tenantInvoices-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="tenantInvoices-datatable">
                    <TenantInvoicesDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <TenantInvoicesEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <TenantInvoicesCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <TenantInvoicesFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <TenantInvoicesSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(TenantInvoicesPage);
