...
const PartnersCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            userId: _entity?.userId?._id,name: _entity?.name,email: _entity?.email,isActive: _entity?.isActive || false,
        ...
        const result = await client.service("partners").create(_data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Partners updated successfully" });
        props.onCreateResult(eagerResult.data[0]);
        } catch (error) {
            console.debug("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Partners" });
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog header="Create Partners" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="partners-create-dialog-component">
...
export default connect(mapState, mapDispatch)(PartnersCreateDialogComponent);
