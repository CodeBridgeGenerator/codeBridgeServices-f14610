...
const PartnersDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const p_booleanTemplate3 = (rowData, { rowIndex }) => <p >{String(rowData.isActive)}</p>
...
<Column field="isActive" header="IsActive" body={p_booleanTemplate3} filter={selectedFilterFields.includes("isActive")} hidden={selectedHideFields?.includes("isActive")}  sortable style={{ minWidth: "8rem" }} />
      ...
      {selectedItems.length > 0 ? (
        ...
        <Dialog header="Upload Partners Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService 
          user={user} 
      ...
      <Dialog header="Search Partners" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
