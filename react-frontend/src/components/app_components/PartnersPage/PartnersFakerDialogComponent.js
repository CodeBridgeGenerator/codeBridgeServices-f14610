...
import partnersFakerFactory from "./partnersFakerFactory";
...
const PartnersFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = partnersFakerFactory(props.user,fakerCount,userIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("partners").create(elem));
...
export default connect(mapState, mapDispatch)(PartnersFakerDialogComponent);
