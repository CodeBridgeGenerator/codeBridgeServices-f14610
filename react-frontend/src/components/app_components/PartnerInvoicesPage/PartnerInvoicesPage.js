...
import PartnerInvoicesDatatable from "./PartnerInvoicesDataTable";
import PartnerInvoicesEditDialogComponent from "./PartnerInvoicesEditDialogComponent";
import PartnerInvoicesCreateDialogComponent from "./PartnerInvoicesCreateDialogComponent";
import PartnerInvoicesFakerDialogComponent from "./PartnerInvoicesFakerDialogComponent";
import PartnerInvoicesSeederDialogComponent from "./PartnerInvoicesSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const PartnerInvoicesPage = (props) => {
    ...
    const filename = "partnerInvoices";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("partnerInvoices");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , partnerId : urlParams.singlePartnersId ,paymentId : urlParams.singlePaymentsId ,tenantInvoices : urlParams.singleTenantInvoicesId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Partner Invoices", type: "error", message: error.message || "Failed get Partner Invoices" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("partnerInvoices").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Partner Invoices", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("partnerInvoices").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/partnerInvoices/${data._id}`);
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>Partner Invoices </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="partnerInvoices-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="partnerInvoices-datatable">
                    <PartnerInvoicesDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <PartnerInvoicesEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <PartnerInvoicesCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <PartnerInvoicesFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <PartnerInvoicesSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(PartnerInvoicesPage);
