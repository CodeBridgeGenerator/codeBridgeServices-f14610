...
import partnerInvoicesFakerFactory from "./partnerInvoicesFakerFactory";
...
const PartnerInvoicesFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [partnerIdIds, setpartnerIdIds] = useState();
const [tenantInvoicesIds, settenantInvoicesIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("partners")
                        ...
                        .then((data) => {
                        setpartnerIdIds(data.data);
                        });
client.service("tenantInvoices")
                        .find({ query: { $select: ["_id","tenantId"] } })
                        .then((data) => {
                        settenantInvoicesIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = partnerInvoicesFakerFactory(props.user,fakerCount,partnerIdIds,tenantInvoicesIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("partnerInvoices").create(elem));
...
export default connect(mapState, mapDispatch)(PartnerInvoicesFakerDialogComponent);
