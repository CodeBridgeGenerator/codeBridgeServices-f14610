...
const PartnerInvoicesCreateDialogComponent = (props) => {
...
const [tenantInvoices, setTenantInvoices] = useState([])
    ...
    useEffect(() => {
        ...
        if (!_.isEmpty(props?.entity)) {
            init = initilization({ ...props?.entity, ...init }, [partnerId,paymentId,tenantInvoices], setError);
        }
        set_entity({...init});
        setError({});
    }, [props.show]);
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            partnerId: _entity?.partnerId?._id,paymentId: _entity?.paymentId?._id,period: _entity?.period,totalAmount: _entity?.totalAmount,paymentStatus: _entity?.paymentStatus,tenantInvoices: _entity?.tenantInvoices?._id,dueDate: _entity?.dueDate,
        ...
        const result = await client.service("partnerInvoices").create(_data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Partner Invoices updated successfully" });
        props.onCreateResult(eagerResult.data[0]);
        } catch (error) {
            console.debug("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Partner Invoices" });
        }
        setLoading(false);
    };
...
const tenantInvoicesOptions = tenantInvoices.map((elem) => ({ name: elem.name, value: elem.value }));
    ...
    return (
        <Dialog header="Create Partner Invoices" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="partnerInvoices-create-dialog-component">
...
export default connect(mapState, mapDispatch)(PartnerInvoicesCreateDialogComponent);
