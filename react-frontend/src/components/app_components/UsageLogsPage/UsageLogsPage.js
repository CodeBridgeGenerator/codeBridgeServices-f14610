...
import UsageLogsDatatable from "./UsageLogsDataTable";
import UsageLogsEditDialogComponent from "./UsageLogsEditDialogComponent";
import UsageLogsCreateDialogComponent from "./UsageLogsCreateDialogComponent";
import UsageLogsFakerDialogComponent from "./UsageLogsFakerDialogComponent";
import UsageLogsSeederDialogComponent from "./UsageLogsSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const UsageLogsPage = (props) => {
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("usageLogs");
    ...
    useEffect(() => {
        ...
        client
            .service("usageLogs")
            .find({ query: { $limit: 10000 , tenantServices : urlParams.singleTenantServicesId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Usage Logs", type: "error", message: error.message || "Failed get Usage Logs" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("usageLogs").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Usage Logs", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("usageLogs").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="usageLogs-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="usageLogs-datatable">
                    <UsageLogsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <UsageLogsEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <UsageLogsCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <UsageLogsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <UsageLogsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(UsageLogsPage);
