...
import usageLogsFakerFactory from "./usageLogsFakerFactory";
...
const UsageLogsFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [tenantServicesIds, settenantServicesIds] = useState();


    useEffect(() => {
        setFakerCount(1);
       
        client.service("tenantServices")
                        .find({ query: { $select: ["_id","serviceId"] } })
                        .then((data) => {
                        settenantServicesIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = usageLogsFakerFactory(props.user,fakerCount,tenantServicesIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("usageLogs").create(elem));
...
export default connect(mapState, mapDispatch)(UsageLogsFakerDialogComponent);
